 ...
 *
 * A POOL rather than one key because PredictStake is a POOL market: the whole
 * point is N independent addresses on both sides of the same question, and the
 * proportional payout can only be checked when several stakes of DIFFERENT
 * sizes sit in the same pool. One key could never exercise it — the contract
 * also blocks the same address from staking both sides, so the both-sides
 * rejection needs two addresses to even be expressible.
 *
...
// `client` deploys and owns both contracts. alice..heidi are predictors, split
// across the YES and NO pools of several markets at different stake sizes so
// the proportional split has something to be wrong about. `resolver` calls
// resolve_market from an address with no position, which is what proves the
// resolver bounty is permissionless rather than a creator privilege;
